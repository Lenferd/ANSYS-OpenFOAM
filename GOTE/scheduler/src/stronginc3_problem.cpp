...
double TStronginC3Problem::CalculateFunctionals(const double *y, int fNumber) {
  ...
  case 0: // constraint 1
    ...
    break;
  case 1: // constraint 2
    res = 10.0 * (1.0 - ((x1 - 2.0) / 1.2) * ((x1 - 2.0) / 1.2) -
                  (x2 / 2.0) * (x2 / 2.0));
    break;
  case 2: // constraint 3
    res = 100.0 * (x2 - 1.5 - 1.5 * sin(6.283 * (x1 - 1.75)));
    break;
  case 3: // criterion
  {
...
// ------------------------------------------------------------------------------------------------
EXPORT_API IProblem *create() { return new TStronginC3Problem(); }

EXPORT_API void destroy(IProblem *ptr) { delete ptr; }
